...
class MaximenuckHelpersourceModule extends CKModel {
	...
	protected static function translate(&$items)
	{
		...
		foreach ($items as $item)
		{
			...
			$item->name = \Maximenuck\CKText::_($item->name);
			if (is_null($item->pages))
			{
				$item->pages = \Maximenuck\CKText::_('JNONE');
			}
			elseif ($item->pages < 0)
			{
				$item->pages = \Maximenuck\CKText::_('COM_MODULES_ASSIGNED_VARIES_EXCEPT');
			}
			elseif ($item->pages > 0)
			{
				$item->pages = \Maximenuck\CKText::_('COM_MODULES_ASSIGNED_VARIES_ONLY');
			}
			else
			{
				$item->pages = \Maximenuck\CKText::_('JALL');
			}
		}
	}
