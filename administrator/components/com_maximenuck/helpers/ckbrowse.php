...
//		}
	...
	private static function getSubfolder($folder, &$tree, $filter, $level) {
		$folders = \Maximenuck\CKFolder::folders($folder, '.', $recurse = false, $fullpath = true);
		natcasesort($folders);
	...
	private static function getImagesInFolder($f, &$tree, $filter, $level) {
			...
			// list all authorized files from the folder
			$files = \Maximenuck\CKFolder::files($f, $filter, $recurse = false, $fullpath = false);
			natcasesort($files);
			$fName = \Maximenuck\CKFile::makeSafe(str_replace(JPATH_SITE, '', $f));
...
//			$name = end($name);
	...
	public static function ajaxAddPicture() {
		...
		if (! \Joomla\CMS\Session\Session::checkToken('get')) {
			$msg = \Maximenuck\CKText::_('JINVALID_TOKEN');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		if (!is_array($file)) {
			$msg = \Maximenuck\CKText::_('CK_NO_FILE_RECEIVED');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		$filename = \Maximenuck\CKFile::makeSafe($file['name']);

		// check the file extension // TODO recup preg_match de local dev
		// if (\Maximenuck\CKFile::getExt($filename) != 'jpg') {
			// $msg = \Maximenuck\CKText::_('CK_NOT_JPG_FILE');
			// echo '{"error" : "'  $msg  '"}';
			// exit;
		// }
		...
		if (!$src || !is_file($src)) {
			$msg = \Maximenuck\CKText::_('CK_FILE_NOT_EXISTS');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		if (!is_dir(JPATH_SITE . $imgpath)) {
			if (!mkdir(JPATH_SITE . $imgpath)) {
				$msg = \Maximenuck\CKText::_('CK_UNABLE_TO_CREATE_FOLDER') . ' : ' . $imgpath;
				echo '{"error" : "' . $msg . '"}';
				exit;
			}
		}
		...
		$object_file = new \Maximenuck\CKObject($file);
		$result = CKFof::triggerEvent('onContentBeforeSave', array('com_media.file', &$object_file, true));

		if (in_array(false, $result, true))
		{
			// There are some errors in the plugins
			throw new Exception(\Maximenuck\CKText::plural('COM_MEDIA_ERROR_BEFORE_SAVE', count($errors = $object_file->getErrors()), implode('<br />', $errors)), 100);

			return false;
		}

		// write the file
		if (! \Maximenuck\CKFile::copy($src, JPATH_SITE . $imgpath . $filename)) {
			$msg = \Maximenuck\CKText::_('CK_UNABLE_WRITE_FILE');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
...
//		$this->setMessage(\Maximenuck\CKText::sprintf('COM_MEDIA_UPLOAD_COMPLETE', substr($object_file->filepath, strlen(COM_MEDIA_BASE))));
