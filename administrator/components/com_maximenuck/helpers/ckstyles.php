...
//				$styles .= $this->genEffects($fields, $prefix, $fullselector);
	...
	private function hasProperty($cssstyles, $prefix) {
		$properties = $cssstyles->$prefix->css['background']
			. $cssstyles->$prefix->css['gradient']
			. $cssstyles->$prefix->css['borders']
			. $cssstyles->$prefix->css['borderradius']
	...
	function genCss($fields, $prefix, $direction) {
		...
		if (isset($fields->$backgroundimageurl) AND $fields->$backgroundimageurl) {
			if ($action == 'preview') {
				$imageurl = substr($fields->$backgroundimageurl, 0, 4)  == 'http' ? $fields->$backgroundimageurl : \Maximenuck\CKUri::root(true) . '/' . $fields->$backgroundimageurl;
			} else {
		...
		if ($gradient0colorvalue && $gradient1colorvalue) {
			...
			if (!\Maximenuck\CKFile::write($svgie9cssdest, $svgie9csstext)) {
				echo '<p class="error">' . \Maximenuck\CKText::_('CK_ERROR_CREATING_SVGIE9CSS') . '</p>';
			}*/
		} else {
			$css['gradient'] = "";
		}
