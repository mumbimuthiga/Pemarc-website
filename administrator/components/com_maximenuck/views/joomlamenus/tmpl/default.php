...
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_maximenuck&view=joomlamenus'.$appendUrl); ?>" method="post" name="adminForm" id="adminForm">
	<?php if ($isModal) { ?>
	<div id="ckheader">
		...
		<div class="ckheadermenu">
			<div class="ckheadertitle">MAXIMENUCK CK - <?php echo \Maximenuck\CKText::_('CK_MENUS_LIST'); ?></div>
		</div>
	</div>
        ...
        <tbody>
			...
			foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					<td>
						<?php if ($isModal) { ?>
						<a href="javascript:void(0)" onclick="window.parent.<?php echo $function ?>('<?php echo $item->menutype; ?>', '<?php echo addslashes($name); ?>')"><?php echo $name; ?></a>
						<?php } else { ?>
						<a href="javascript:void(0)" onclick="CKBox.open({handler:'iframe', fullscreen: true, url:'<?php echo \Maximenuck\CKUri::root(true) . '/administrator/' . $link ?>'})"><?php echo $name; ?></a>
						<?php } ?>
