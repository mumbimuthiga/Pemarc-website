...
<div class="ckrow" style="margin-left:200px;">
	<label><?php echo \Maximenuck\CKText::_('CK_ORIENTATION'); ?></label>
	...
	<div class="ckbutton-group">
		...
		<label class="ckbutton first" for="orientationhorizontal"><?php echo \Maximenuck\CKText::_('CK_HORIZONTAL'); ?></label>
		...
		<label class="ckbutton"  for="orientationvertical"><?php echo \Maximenuck\CKText::_('CK_VERTICAL'); ?></label>
	</div>
</div>
<hr />
<?php
$path = JPATH_ROOT . '/modules/mod_maximenuck/tmpl';
$files = \Maximenuck\CKFolder::files($path, '.php');
natsort($files);
$i = 1;
...
foreach ($files as $file) {
	...
	$thumb_title = '';
	$file = \Maximenuck\CKFile::stripExt($file);
	if ($file === 'deault2') continue;
	if ( file_exists($path . '/' . $file. '.png') ) {
		$thumb = \Maximenuck\CKUri::root(true) . '/modules/mod_maximenuck/tmpl/' . $file . '.png';
	} else {
		$thumb = \Maximenuck\CKUri::root(true) . '/administrator/components/com_maximenuck/images/what.png style="display:block;margin:0 auto;" width="90 height="90';
		$thumb_title = \Maximenuck\CKText::_('CK_LAYOUT_PREVIEW_NOT_FOUND');
	}
