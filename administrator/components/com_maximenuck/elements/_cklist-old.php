...
class JFormFieldCklist extends \Joomla\CMS\Form\FormField {
	...
	protected function getPathToElements() {
		...
		$rootpath = JPATH_ROOT;
		$httppath = trim(\Maximenuck\CKUri::root(), "/");
	...
	protected function getOptions() {
		...
		foreach ($this->element->children() as $option) {
			...
			$tmp = \Joomla\CMS\HTML\HTMLHelper::_(
							'select.option', (string) $option['value'], \Maximenuck\CKText::alt(trim((string) $option), preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->fieldname)), 'value', 'text', ((string) $option['disabled'] == 'true')
			);
	...
	protected function getLabel() {
		...
		if (!empty($this->description)) {
			$label .= ' title="'
					. htmlspecialchars(
							trim($text, ':') . '::' . ($this->translateDescription ? \Maximenuck\CKText::_($this->description) : $this->description), ENT_COMPAT, 'UTF-8'
					) . '"';
		}
