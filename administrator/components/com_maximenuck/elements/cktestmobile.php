...
class JFormFieldCktestmobile extends \Joomla\CMS\Form\FormField {
    ...
    protected function getInput() {
        ...
        if ((string) $this->element['hr'] == 'true') {
            ...
            $text = $this->translateLabel ? \Maximenuck\CKText::_($text) : $text;
            ...
            if (!empty($this->description)) {
                $label .= ' title="' . htmlspecialchars(trim($text, ':') . '::' .
                    ($this->translateDescription ? \Maximenuck\CKText::_($this->description) : $this->description), ENT_COMPAT, 'UTF-8') . '"';
            }
	...
	protected function testPatch($component) {
		if (file_exists(JPATH_ROOT . '/plugins/system/' . $component .'/'. $component . '.php')
				&& \Joomla\CMS\Plugin\PluginHelper::isEnabled('system',$component)) {
			$this->element['icon'] = 'info';
			$this->element['iconcolor'] = 'green';
			return \Maximenuck\CKText::_('MOD_MAXIMENUCK_SPACER_' . strtoupper($component) . '_PATCH_INSTALLED');
		} else {
			$this->element['icon'] = 'exclamation';
			$this->element['iconcolor'] = 'orange';
			return \Maximenuck\CKText::_('MOD_MAXIMENUCK_CHECK_MOBILEMENUCK') . ' <a href="https://www.joomlack.fr/en/joomla-extensions/mobile-menu-ck" target="_blank">Mobile Menu CK</a>';
		}
		return false;
	}
}