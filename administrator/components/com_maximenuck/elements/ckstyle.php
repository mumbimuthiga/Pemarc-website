...
\Maximenuck\CKText::script('MAXIMENUCK_SAVE_CLOSE');
	...
	protected function getInput() {
		...
		if (Helper::isV9() == '0') {
			...
			$html = '<input name="' 
				...
				. \Maximenuck\CKText::_('MAXIMENUCK_STYLES_WIZARD') . '" onclick="CKBox.open({handler:\'iframe\', fullscreen: true, url:\'' 
				. \Maximenuck\CKUri::root(true) . '/administrator/index.php?option=com_maximenuck&view=style&layout=edit&tmpl=component&frommoduleid=' 
				. \Maximenuck\CKFof::getApplication()->input->get('id',0,'int')
				// . '&id=' . $this->value
				.'\'})"/>';
			...
			$html .= '<input type="text" disabled name="' . $this->name . 'name" id="' . $this->id . 'name"' . ' value="'
				. htmlspecialchars((string)$styleName) . '"' . $class . $size . $disabled . $readonly . $onchange . $maxLength . $styles . '/>';
			$footerHtml = '<a class="ckboxmodal-button" href="javascript:void(0)" onclick="ckSaveIframe(\'test\')">' . \Maximenuck\CKText::_('CK_CREATE_NEW') . '</a>';
			$html .= '<div class="ckbutton" onclick="CKBox.open({id: \'ckstylesmodal\', url: \'index.php?option=com_maximenuck&view=styles&tmpl=component&layout=modal\', style: {padding: \'0px\'}})"><i class="fas fa-mouse-pointer "></i> ' . \Maximenuck\CKText::_('CK_SELECT') . '</div>';
			$html .= '<div class="ckbutton" onclick="CKBox.open({url: \'index.php?option=com_maximenuck&view=style&tmpl=component&layout=edit&id=\'+jQuery(\'#' . $this->id . '\').val()+\'\'})"><i class="fas fa-edit"></i> ' . \Maximenuck\CKText::_('CK_EDIT') . '</div>';
			$html .= '<div class="ckbutton cktip" onclick="jQuery(\'#' . $this->id . '\').val(\'\');jQuery(\'#' . $this->id . 'name\').val(\'\');" title="' . \Maximenuck\CKText::_('CK_REMOVE') . '"><i class="fas fa-times"></i></div>';
			$html .= '</div>';
