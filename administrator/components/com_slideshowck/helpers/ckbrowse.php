...
class CKBrowse {
	...
	private static function createFolderObj($f, &$tree, $level) {
		$fName = Slideshowck\CKFile::makeSafe(str_replace(JPATH_SITE, '', $f));
	...
	public static function ajaxAddPicture() {
		...
		if (! \Joomla\CMS\Session\Session::checkToken('get')) {
			$msg = Slideshowck\CKText::_('JINVALID_TOKEN');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		if (!is_array($file)) {
			$msg = Slideshowck\CKText::_('CK_NO_FILE_RECEIVED');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}

		$filename = Slideshowck\CKFile::makeSafe($file['name']);

		// check the file extension // TODO recup preg_match de local dev
		// if (Slideshowck\CKFile::getExt($filename) != 'jpg') {
			// $msg = Slideshowck\CKText::_('CK_NOT_JPG_FILE');
			// echo '{"error" : "'  $msg  '"}';
			// exit;
		// }
		...
		if (!$src || !is_file($src)) {
			$msg = Slideshowck\CKText::_('CK_FILE_NOT_EXISTS');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		if (!is_dir(JPATH_SITE . $imgpath)) {
			if (!mkdir(JPATH_SITE . $imgpath)) {
				$msg = Slideshowck\CKText::_('CK_UNABLE_TO_CREATE_FOLDER') . ' : ' . $imgpath;
				echo '{"error" : "' . $msg . '"}';
				exit;
			}
		}
		...
		// write the file
		if (! Slideshowck\CKFile::copy($src, JPATH_SITE . $imgpath . $filename)) {
			$msg = Slideshowck\CKText::_('CK_UNABLE_WRITE_FILE');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
