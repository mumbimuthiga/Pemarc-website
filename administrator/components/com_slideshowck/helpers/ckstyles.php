...
//		}
	...
	function genCss($fields, $prefix, $direction) {
		...
		if (isset($fields->$backgroundimageurl) AND $fields->$backgroundimageurl) {
			if ($action == 'preview') {
				$imageurl = substr($fields->$backgroundimageurl, 0, 4)  == 'http' ? $fields->$backgroundimageurl : Slideshowck\CKUri::root(true) . '/' . $fields->$backgroundimageurl;
			} else {
		...
		if ($gradient0colorvalue && $gradient1colorvalue) {
			...
			$svgie9csstext = '<?xml version="1.0" ?>
              ...
              ';
			if (!Slideshowck\CKFile::write($svgie9cssdest, $svgie9csstext)) {
				echo '<p class="error">' . Slideshowck\CKText::_('CK_ERROR_CREATING_SVGIE9CSS') . '</p>';
			}*/
		} else {
			$css['gradient'] = "";
		}
