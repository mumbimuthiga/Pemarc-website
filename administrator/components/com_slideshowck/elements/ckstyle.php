...
Slideshowck\CKText::script('SLIDESHOWCK_SAVE_CLOSE');
...
class JFormFieldCkstyle extends CKFormField {
	...
	protected function getInput() {
		...
		$footerHtml = '<a class="ckboxmodal-button" href="javascript:void(0)" onclick="ckSaveIframe(\'test\')">' . Slideshowck\CKText::_('CK_CREATE_NEW') . '</a>';
		$html .= '<div class="ckbutton" onclick="CKBox.open({id: \'ckstylesmodal\', url: \'index.php?option=com_slideshowck&view=styles&tmpl=component&layout=modal\', style: {padding: \'0px\'}})"><i class="fas fa-mouse-pointer "></i> ' . Slideshowck\CKText::_('SLIDESHOWCK_SELECT') . '</div>';
		$html .= '<div class="ckbutton" onclick="CKBox.open({url: \'index.php?option=com_slideshowck&view=style&tmpl=component&layout=modal&id=\'+jQuery(\'#' . $this->id . '\').val()+\'\'})"><i class="fas fa-edit"></i> ' . Slideshowck\CKText::_('SLIDESHOWCK_EDIT') . '</div>';
		$html .= '<div class="ckbutton cktip" onclick="jQuery(\'#' . $this->id . '\').val(\'\');jQuery(\'#' . $this->id . 'name\').val(\'\');" title="' . Slideshowck\CKText::_('SLIDESHOWCK_REMOVE') . '"><i class="fas fa-times"></i></div>';
		$html .= '</div>';
