...
class modSlideshowckHelper {
	 ...
	 *
	 * @param	Slideshowck\CKRegistry	$params	The module options.
	 *
	 * @return	array
	 */
	static function getItems(&$params) {
		...
		$document = Slideshowck\CKFof::getDocument();
		...
		foreach ($items as $i => $item) {
			...
			if (stristr($item->imgname, "http")) {
				...
				if ($params->get('thumbnails', '1') == '1' && $params->get('autocreatethumbs','1')) {
					$item->imgthumb = Slideshowck\CKUri::base(true) . '/' . self::resizeImage($item->imgname, $params->get('thumbnailwidth', '182'), $params->get('thumbnailheight', '187'));
				} else {
					$thumbfile = str_replace(Slideshowck\CKFile::getName($item->imgname), "th/" . Slideshowck\CKFile::getName($item->imgname), $item->imgname);
					$thumbfile = str_replace("." . $thumbext, "_th." . $thumbext, $thumbfile);
					$item->imgthumb = Slideshowck\CKUri::base(true) . '/' . $thumbfile;
				}
				$item->imgname = Slideshowck\CKUri::base(true) . '/' . $item->imgname;
			}
...
//			$item->imgname = str_replace(Slideshowck\CKUri::base(), '', $item->imgname);
			$item->imgthumb = '';
			...
			if ($params->get('thumbnails', '1') == '1')
				$item->imgthumb = Slideshowck\CKUri::base(true) . '/' . self::resizeImage($item->imgname, $params->get('thumbnailwidth', '100'), $params->get('thumbnailheight', '75'));
			$thumbext = explode(".", $item->imgname);
			$thumbext = end($thumbext);
			// set the variables
			$item->imgvideo = null;
			...
			if (!in_array(strToLower(Slideshowck\CKFile::getExt($item->imgname)), $authorisedExt))
				continue;
	...
	static function getItemsAutoloadfolder(&$params) {
		...
		if (file_exists($folder . '/labels.txt')) {
			...
			$items = Slideshowck\CKFolder::files($folder, '.jpg|.png|.jpeg|.gif|.JPG|.JPEG|.jpeg', false, true);
	...
	public static function loadImagesFromFolder($directory) {
		...
		$files = Slideshowck\CKFolder::files(trim(trim($directory), '/'), '.', false, true);
...
//		}
		...
		foreach ($files as $file) {
			$fileExt = Slideshowck\CKFile::getExt($file);
...
//			if (isset($params->number) && $params->number > 0 && $i > (int)$params->number) $show = false;
			...
			$item->imgname = Slideshowck\CKUri::base(true) . '/' . $file;
			$item->imgthumb = $item->imgname;
...
//			}
			...
			$registry = new Slideshowck\CKRegistry;
...
//				$slideItem->imgname = trim(str_replace('\\', '/', $item->images['image_intro']), '/');
				...
				$slideItem->imgthumb = Slideshowck\CKUri::base(true) . '/' . $slideItem->imgname;
				$slideItem->imgname = Slideshowck\CKUri::base(true) . '/' . $slideItem->imgname;
	...
	static function getImageDataFromfolder(&$item, $params) {
		...
		$item->imglink = null;
		// load the image data from txt
		$datafile = JPATH_ROOT . '/' . str_replace(Slideshowck\CKFile::getExt($item->imgname), 'txt', $item->imgname);
		$data = is_file($datafile) ? file_get_contents($datafile) : '';
		$imgdatatmp = explode("\n", $data);
	...
	static function resizeImage($file, $x, $y = '', $thumbpath = 'th', $thumbsuffix = '_th') {
		...
		$thumbext = end($thumbext);
		$thumbfile = str_replace(Slideshowck\CKFile::getName($file), $thumbpath . "/" . Slideshowck\CKFile::getName($file), $file);
...
//			$y = $tmpx * $size[0] / $size[1];
		...
		if ($size) {
			...
			$thumbfolder = str_replace(Slideshowck\CKFile::getName($file), $thumbpath . "/", $filetmp);
	 ...
	 * Create the css
	 *
	 * $params Slideshowck\CKRegistry the module params
...
//		$css['background'] = ($params->get($prefix . 'bgcolor1') AND $params->get($prefix . 'usebackground')) ? 'background: ' . $bgcolor1 . ';' : '';
		$css['background'] .= ( $params->get($prefix . 'bgimage') AND $params->get($prefix . 'usebackground')) ? 'background-image: url("' . Slideshowck\CKUri::ROOT() . $params->get($prefix . 'bgimage') . '");' : '';
