...
//		if (!$model->copy()) {
//			$msg = Slideshowck\CKText::_('CK_COPY_ERROR');
//			$type = 'error';
//		} else {
//			$msg = Slideshowck\CKText::_('CK_COPY_SUCCESS');
//			$type = 'message';
//		}
//
...
//	}
	...
	public function uploadParamsFile() {
		...
		if (Slideshowck\CKFile::getExt($filename) != 'mmck') {
			$msg = Slideshowck\CKText::_('CK_NOT_MMCK_FILE', true);
		...
		// check if the file exists
		if (!$src || !is_file($src)) {
			$msg = Slideshowck\CKText::_('CK_FILE_NOT_EXISTS', true);
			echo json_encode(array('error'=> $msg));
			exit();
		}

		// read the file
		if (!$filecontent = Slideshowck\CKFile::read($src)) {
			$msg = Slideshowck\CKText::_('CK_UNABLE_READ_FILE', true);
			echo json_encode(array('error'=> $msg));
			exit();
		}

		// replace vars to allow data to be moved from another server
		$filecontent = str_replace("|URIROOT|", Slideshowck\CKUri::root(true), $filecontent);
