...
class CKBrowse {
	...
	private static function getSubfolder($folder, &$tree, $filter, $level) {
		$folders = \Mobilemenuck\CKFolder::folders($folder, '.', $recurse = false, $fullpath = true);
		natcasesort($folders);
	...
	private static function getImagesInFolder($f, &$tree, $filter, $level) {
			...
			// list all authorized files from the folder
			$files = \Mobilemenuck\CKFolder::files($f, $filter, $recurse = false, $fullpath = false);
			natcasesort($files);
			$fName = \Mobilemenuck\CKFile::makeSafe(str_replace(JPATH_SITE, '', $f));
	...
	public static function ajaxAddPicture() {
		...
		if (! \Joomla\CMS\Session\Session::checkToken('get')) {
			$msg = \Mobilemenuck\CKText::_('JINVALID_TOKEN');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		if (!is_array($file)) {
			$msg = \Mobilemenuck\CKText::_('CK_NO_FILE_RECEIVED');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		// if (\Mobilemenuck\CKFile::getExt($filename) != 'jpg') {
			// $msg = \Mobilemenuck\CKText::_('CK_NOT_JPG_FILE');
			// echo '{"error" : "'  $msg  '"}';
			// exit;
		// }
		...
		if (!$src || !is_file($src)) {
			$msg = \Mobilemenuck\CKText::_('CK_FILE_NOT_EXISTS');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		if (!is_dir(JPATH_SITE . $imgpath)) {
			if (!mkdir(JPATH_SITE . $imgpath)) {
				$msg = \Mobilemenuck\CKText::_('CK_UNABLE_TO_CREATE_FOLDER') . ' : ' . $imgpath;
				echo '{"error" : "' . $msg . '"}';
				exit;
			}
		}
		...
		// write the file
		if (! \Mobilemenuck\CKFile::copy($src, JPATH_SITE . $imgpath . $filename)) {
			$msg = \Mobilemenuck\CKText::_('CK_UNABLE_WRITE_FILE');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
