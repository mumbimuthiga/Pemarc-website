...
";
			...
			if (
					...
					|| $cssstyles->$prefix->css['textshadow']
						...
						$styles .= "
" . $fullselector . " a, " . $fullselector . " span.separator, " . $fullselector . " span.nav-header {
"
					. $cssstyles->$prefix->css['color']
					. $cssstyles->$prefix->css['fontbold']
					. $cssstyles->$prefix->css['fontitalic']
...
";
	...
	function genCss($fields, $prefix, $direction) {
		...
		if (isset($fields->$backgroundimageurl) AND $fields->$backgroundimageurl) {
			if ($action == 'preview') {
				$imageurl = substr($fields->$backgroundimageurl, 0, 4)  == 'http' ? $fields->$backgroundimageurl : \Mobilemenuck\CKUri::root(true) . '/' . $fields->$backgroundimageurl;
			} else {
		...
		$backgroundimageleftvalue = (isset($fields->$backgroundimageleft) AND $fields->$backgroundimageleft != null) ? trim($fields->$backgroundimageleft) : "center";
		$backgroundimagetopvalue = (isset($fields->$backgroundimagetop) AND $fields->$backgroundimagetop != null) ? trim($fields->$backgroundimagetop) : "center";
