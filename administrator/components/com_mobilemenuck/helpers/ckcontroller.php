...
class CKController {
	...
	function __construct() {
		...
		$hasModal = $this->input->get('modal', '', 'string') == '1';
		$appendUrl = '';
		if ($isModal) {
			$appendUrl = '&layout=modal&tmpl=component';
		} else if ($hasModal) {
			$appendUrl = '&modal=1&tmpl=component';
	}
	...
	static function getInstance($prefix) {
		...
		$basePath = MOBILEMENUCK_BASE_PATH;
		...
		if (!class_exists($class))
		{
			...
			{
				throw new \InvalidArgumentException(\Mobilemenuck\CKText::sprintf('ERROR_INVALID_CONTROLLER', $type, $format));
			}
		}
		...
		if (!class_exists($class))
		{
			throw new \InvalidArgumentException(\Mobilemenuck\CKText::sprintf('ERROR_INVALID_CONTROLLER_CLASS', $class));
		}
	...
	// public function getModel($base = '\Mobilemenuck\CKModel') {
		// if (empty($this->model)) {
			// $name = $this->getName();
			// require_once(MOBILEMENUCK_BASE_PATH . '/helpers/ckmodel.php');
			// require_once(MOBILEMENUCK_BASE_PATH . '/models/' . strtolower($name) . '.php');
	...
	public function getView($name = '', $type = 'html', $prefix = '')
	{
		...
		if (empty(self::$views[$name][$type][$prefix]))
		{
			...
			{
				throw new \Exception(\Mobilemenuck\CKText::sprintf('ERROR_VIEW_NOT_FOUND', $name, $type, $prefix), 404);
			}
		}
	...
	protected function createView($name, $prefix = '')
	{
		...
		{
			$path = MOBILEMENUCK_BASE_PATH . '/views/' . $this->createFileName('view', array('name' => $viewName));
			...
			if (!class_exists($viewClass))
			{
				throw new \Exception(\Mobilemenuck\CKText::_('ERROR_VIEW_CLASS_NOT_FOUND : ' . $viewClass . ' - ' . $path), 500);
			}
		}
	...
	public function execute($task) {
		...
		{
			throw new \Exception(\Mobilemenuck\CKText::sprintf('ERROR_TASK_NOT_FOUND', $task), 404);
		}
	...
	public function edit($id = null, $appendUrl = '') {
		...
		CKFof::redirect(MOBILEMENUCK_ADMIN_GENERAL_URL . '&view=' . $this->getName() . '&layout=default&id=' . $editId . $appendUrl);
	}
	...
	public function copy() {
		...
		if (! $this->redirect) $this->redirect = MOBILEMENUCK_ADMIN_GENERAL_URL;
