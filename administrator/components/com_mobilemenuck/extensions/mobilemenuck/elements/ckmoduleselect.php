...
function ckMobilemenuUpdateModule(nothing, id, name) {
		...
		$html .= '<div class="btn btn-outline-secondary" onclick="CKBox.open({url: \'index.php?option=com_mobilemenuck&view=items&tmpl=component&layout=select&returnFunc=ckMobilemenuSelectModule\'})"><i class="fas fa-mouse-pointer"></i> ' . \Mobilemenuck\CKText::_('PLG_MOBILEMENUCK_SELECT') . '</div>';
//		$html .= '<div class="btn btn-secondary" onclick="if (jQuery(\'#' . $this->id . '\').val()) {CKBox.open({url: \'index.php?option=com_mobilemenuck&view=style&tmpl=component&modal=1&id=\'+jQuery(\'#' . $this->id . '\').val()+\'\'}) } else { alert(\'' . \Mobilemenuck\CKText::_('PLG_MOBILEMENUCK_SELECT_FIRST', true) . '\');}">' . \Mobilemenuck\CKText::_('PLG_MOBILEMENUCK_EDIT', true) . '</div>';
		$html .= '<div class="btn btn-outline-secondary" onclick="jQuery(\'#' . $this->id . '\').val(\'\').trigger(\'change\');jQuery(\'#' . $this->id . 'name\').val(\'\');"><i class="fas fa-times"></i> ' . \Mobilemenuck\CKText::_('PLG_MOBILEMENUCK_CLEAR', true) . '</div>';
		$html .= '</div>';
