...
\Sliderck\CKText::script('SLIDERCK_SAVE_CLOSE');
...
class JFormFieldCkstyle extends \Joomla\CMS\Form\FormField {
	...
	protected function getInput() {
		...
		$footerHtml = '<a class="ckboxmodal-button" href="javascript:void(0)" onclick="ckSaveIframe(\'test\')">' . \Sliderck\CKText::_('CK_CREATE_NEW') . '</a>';
		$html .= '<div class="ckbutton" onclick="CKBox.open({url: \'index.php?option=com_sliderck&view=styles&tmpl=component&layout=modal\', style: {padding: \'0px\'}})"><i class="fas fa-mouse-pointer "></i> ' . \Sliderck\CKText::_('SLIDERCK_SELECT') . '</div>';
		$html .= '<div class="ckbutton" onclick="CKBox.open({url: \'index.php?option=com_sliderck&view=style&tmpl=component&layout=modal&id=\'+jQuery(\'#' . $this->id . '\').val()+\'\'})"><i class="fas fa-edit"></i> ' . \Sliderck\CKText::_('SLIDERCK_EDIT') . '</div>';
		$html .= '<div class="ckbutton cktip" onclick="jQuery(\'#' . $this->id . '\').val(\'\');jQuery(\'#' . $this->id . 'name\').val(\'\');" title="' . \Sliderck\CKText::_('SLIDERCK_REMOVE') . '"><i class="fas fa-times"></i></div>';
		$html .= '</div>';
