...
class JFormFieldCksource extends \Joomla\CMS\Form\Field\ListField
{
	...
	protected function getOptions() {
		...
		if (\Joomla\CMS\Plugin\PluginHelper::isEnabled('system', 'sliderck')) {
			// load the custom plugins
			...
			if (count($sources)) {
				foreach ($sources as $source) {
					...
					if (! $this->findOption($source)) {
						$tmp = \Joomla\CMS\HTML\HTMLHelper::_(
							'select.option', (string) $source, \Sliderck\CKText::alt(trim((string) 'SLIDERCK_SOURCE_' . strtoupper($source)), preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->fieldname)), 'value', 'text', '0'
						);
						// Add the option object to the result set.
						$this->options[] = $tmp;
					}
