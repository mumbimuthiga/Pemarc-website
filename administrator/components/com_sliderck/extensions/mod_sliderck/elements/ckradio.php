...
class JFormFieldCkradio extends \Joomla\CMS\Form\FormField {
    ...
    protected function getInput() {
		...
		foreach ($options as $i => $option)
		{
			...
			$html[] = '<label for="' . $this->id . $i . '"' . $class . '>'
				. \Sliderck\CKText::alt($option->text, preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->fieldname)) . '</label>';
		}
	...
	protected function getLabel()
	{
		...
		if (!empty($this->description)) {
			$label .= ' title="<strong>'
					. htmlspecialchars(
							trim($text, ':') . '</strong><br />' . ($this->translateDescription ? \Sliderck\CKText::_($this->description) : $this->description), ENT_COMPAT, 'UTF-8'
					) . '"';
		}
