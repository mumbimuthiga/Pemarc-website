...
class modCarouselckHelper {
	 ...
	 *
	 * @param	\Carouselck\CKRegistry	$params	The module options.
	 *
	 * @return	array
	 */
	static function getItems(&$params) {
		...
		self::$_params = $params;
		$db = \Carouselck\CKFof::getDbo();
		$document = \Carouselck\CKFof::getDocument();
		...
		foreach ($items as $i => $item) {
			...
			if (stristr($item->imgname, "http")) {
				...
				if ($params->get('thumbnails', '1') == '1' && $params->get('autocreatethumbs','1')) {
					$item->imgthumb = \Carouselck\CKUri::base(true) . '/' . self::resizeImage($item->imgname, $params->get('thumbnailwidth', '182'), $params->get('thumbnailheight', '187'));
				} else {
					$thumbfile = str_replace(\Carouselck\CKFile::getName($item->imgname), "th/" . \Carouselck\CKFile::getName($item->imgname), $item->imgname);
					$thumbfile = str_replace("." . $thumbext, "_th." . $thumbext, $thumbfile);
					$item->imgthumb = \Carouselck\CKUri::base(true) . '/' . $thumbfile;
				}
				$item->imgname = \Carouselck\CKUri::base(true) . '/' . $item->imgname;
			}
	...
	static function getArticle(&$item, $params) {
		...
		$app = \Carouselck\CKFof::getApplication();
...
//			$item->imgname = str_replace(\Carouselck\CKUri::base(), '', $item->imgname);
			$item->imgthumb = '';
			...
			if ($params->get('thumbnails', '1') == '1')
				$item->imgthumb = \Carouselck\CKUri::base(true) . '/' . self::resizeImage($item->imgname, $params->get('thumbnailwidth', '100'), $params->get('thumbnailheight', '75'));
			$thumbext = explode(".", $item->imgname);
			$thumbext = end($thumbext);
			// set the variables
			$item->imgvideo = null;
			...
			if (!in_array(strToLower(\Carouselck\CKFile::getExt($item->imgname)), $authorisedExt))
				continue;
	...
	static function getItemsAutoloadfolder(&$params) {
		...
		if (file_exists($folder . '/labels.txt')) {
			...
			$items = \Carouselck\CKFolder::files($folder, '.jpg|.png|.jpeg|.gif|.JPG|.JPEG|.jpeg', false, true);
	...
	public static function loadImagesFromFolder($directory) {
		...
		$files = \Carouselck\CKFolder::files(trim(trim($directory), '/'), '.', false, true);
...
//		}
		...
		foreach ($files as $file) {
			$fileExt = \Carouselck\CKFile::getExt($file);
...
//			if (isset($params->number) && $params->number > 0 && $i > (int)$params->number) $show = false;
			...
			$item->imgname = \Carouselck\CKUri::base(true) . '/' . $file;
			$item->imgthumb = $item->imgname;
...
//			}
			...
			$registry = new \Carouselck\CKRegistry;
...
//				$slideItem->imgname = trim(str_replace('\\', '/', $item->images['image_intro']), '/');
				...
				$slideItem->imgthumb = \Carouselck\CKUri::base(true) . '/' . $slideItem->imgname;
				$slideItem->imgname = \Carouselck\CKUri::base(true) . '/' . $slideItem->imgname;
	...
	static function getImageDataFromfolder(&$item, $params) {
		...
		$item->imglink = null;
		// load the image data from txt
		$datafile = JPATH_ROOT . '/' . str_replace(\Carouselck\CKFile::getExt($item->imgname), 'txt', $item->imgname);
		$data = is_file($datafile) ? \Carouselck\CKFile::read($datafile) : '';
		$imgdatatmp = explode("\n", $data);
	...
	static function resizeImage($file, $x, $y = '', $thumbpath = 'th', $thumbsuffix = '_th') {
		...
		$thumbext = end($thumbext);
		$thumbfile = str_replace(\Carouselck\CKFile::getName($file), $thumbpath . "/" . \Carouselck\CKFile::getName($file), $file);
...
//			$y = $tmpx * $size[0] / $size[1];
		...
		if ($size) {
			...
			$thumbfolder = str_replace(\Carouselck\CKFile::getName($file), $thumbpath . "/", $filetmp);
			if (!is_dir($thumbfolder)) { 
				mkdir($thumbfolder);
				\Carouselck\CKFile::copy(JPATH_ROOT . '/modules/mod_carouselck/index.html', $thumbfolder . 'index.html' );
			}
	 ...
	 * Create the css
	 *
	 * $params \Carouselck\CKRegistry the module params
...
//		$css['background'] = ($params->get($prefix . 'bgcolor1') AND $params->get($prefix . 'usebackground')) ? 'background: ' . $bgcolor1 . ';' : '';
		$css['background'] .= ( $params->get($prefix . 'bgimage') AND $params->get($prefix . 'usebackground')) ? 'background-image: url("' . \Carouselck\CKUri::ROOT() . $params->get($prefix . 'bgimage') . '");' : '';
