...
class CKBrowse {
	...
	public static function getItemsList($type = 'image') {
		...
		$folder = $input->get('folder', '', 'string') ? '/' . trim($input->get('folder', '', 'string'), '/') : '/' . trim(\Joomla\CMS\Component\ComponentHelper::getParams('com_carouselck')->get('imagespath', 'images/carouselck'), '/');
		...
		// no folder filtering 
		if (\Joomla\CMS\Component\ComponentHelper::getParams('com_carouselck')->get('imagespathexclusive', '0') == '0') {
	...
	private static function getSubfolder($folder, &$tree, $filter, $level) {
		$folders = \Carouselck\CKFolder::folders($folder, '.', $recurse = false, $fullpath = true);
		natcasesort($folders);
	...
	private static function createFolderObj($f, &$tree, $level) {
			$fName = \Carouselck\CKFile::makeSafe(str_replace(JPATH_SITE, '', $f));
...
//			$name = end($name);
	...
	public static function getImagesInFolder($f, $filter = '.') {
			...
			// list all authorized files from the folder
			$files = \Carouselck\CKFolder::files($f, $filter, $recurse = false, $fullpath = false);
	...
	public static function ajaxAddPicture() {
		...
		if (! \Joomla\CMS\Session\Session::checkToken('get')) {
			$msg = \Carouselck\CKText::_('JINVALID_TOKEN');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		$imgpath = $input->get('path', '', 'string') ? '/' . trim($input->get('path', '', 'string'), '/') . '/' : '/' . trim(\Joomla\CMS\Component\ComponentHelper::getParams('com_carouselck')->get('imagespath', 'images/carouselck'), '/') . '/';
		...
		if (!is_array($file)) {
			$msg = \Carouselck\CKText::_('CK_NO_FILE_RECEIVED');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}

		$filename = \Carouselck\CKFile::makeSafe($file['name']);

		// check the file extension // TODO recup preg_match de local dev
		// if (\Carouselck\CKFile::getExt($filename) != 'jpg') {
			// $msg = \Carouselck\CKText::_('CK_NOT_JPG_FILE');
			// echo '{"error" : "'  $msg  '"}';
			// exit;
		// }
		...
		if (!$src || !is_file($src)) {
			$msg = \Carouselck\CKText::_('CK_FILE_NOT_EXISTS');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
		...
		if (!is_dir(JPATH_SITE . $imgpath)) {
			if (!mkdir(JPATH_SITE . $imgpath)) {
				$msg = \Carouselck\CKText::_('CK_UNABLE_TO_CREATE_FOLDER') . ' : ' . $imgpath;
				echo '{"error" : "' . $msg . '"}';
				exit;
			}
		}
		...
		// write the file
		if (! \Carouselck\CKFile::copy($src, JPATH_SITE . $imgpath . $filename)) {
			$msg = \Carouselck\CKText::_('CK_UNABLE_WRITE_FILE');
			echo '{"error" : "' . $msg . '"}';
			exit;
		}
