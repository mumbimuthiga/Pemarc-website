...
<script type="text/javascript">
	var URIROOT = "<?php echo \Carouselck\CKUri::root(true); ?>";
	var URIBASE = "<?php echo \Carouselck\CKUri::base(true); ?>";
...
<div id="ckfolderupload">
	<div class="inner">
		<div class="upload">
			<h2 class="uploadinstructions"><?php echo \Carouselck\CKText::_( 'Drop files here to upload' ); ?></h2>
			<p><?php echo \Carouselck\CKText::_( 'or Select Files' ); ?></p><input id="ckfileupload" type="file" class="" />
		</div>
	</div>
</div>
<div id="ckfoldertreelist">
<p><?php echo \Carouselck\CKText::_('CK_BROWSE_INFOS') ?></p>
<h3><?php echo \Carouselck\CKText::_('CK_FOLDERS') ?></h3>
<?php
$lastitem = 0;
	...
	<div class="ckfoldertree <?php echo $folderclass ?> <?php echo ($folder->deeper ? 'parent' : '') ?> <?php //echo (count($folder->files) ? 'hasfiles' : '') ?>" data-level="<?php echo $folder->level ?>" data-path="<?php echo ($folder->basepath) ?>">
		...
		<div class="ckfoldertreefiles">
			...
			<div id="ckfoldertreelistfolderselection">
				<div class="ckbutton ckbutton-primary" style="font-size:20px;padding: 10px 20px;" onclick="ckSelectFolder('<?php echo ($folder->basepath) ?>')"><i class="fas fa-check-square"></i> <?php echo \Carouselck\CKText::_('CK_SELECT_FOLDER') ?><br /><small><?php echo $pathway ?></small></div>
			</div>
			<?php } ?>
			<div class="ckfoldertreepathway ckinterface">
				...
				<span class="ckfoldertreepathwayactions">
					<span class="ckfoldertreepathwayaddfolder ckbutton" onclick="ckAddFolder()"><?php echo \Carouselck\CKText::_('CK_ADD_SUB_FOLDER') ?></span>
					<span class="ckfoldertreepathwayfoldername"><input type="text" class="ckfoldertreepathwayaddfoldername" /></span>
					<span class="ckfoldertreepathwaycreatefolder ckbutton" onclick="ckCreateFolder(this, '<?php echo ($folder->basepath) ?>')"><?php echo \Carouselck\CKText::_('CK_CREATE_FOLDER') ?></span>
				</span>
				<?php } ?>
			</div>
		...
		?>
				<div class="ckfoldertreefile ckwait" data-type="<?php echo $type ?>" onclick="ckSelectFile(this)" data-path="<?php echo ($folder->basepath) ?>" data-filename="<?php echo ($file) ?>">
					<div class="ckfakeimage" data-src="<?php echo \Carouselck\CKUri::root(true) . '/' . ($folder->basepath) . '/' . ($file) ?>" title="<?php echo ($file); ?>" ></div>
...
var URIROOT = window.URIROOT || '<?php echo \Carouselck\CKUri::root(true) ?>';
