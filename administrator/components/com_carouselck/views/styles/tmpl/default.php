...
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_carouselck&view=styles'.$appendUrl); ?>" method="post" name="adminForm" id="adminForm">
	<?php if ($isModal) { ?>
	<div id="ckheader">
		...
		<div class="ckheadermenu">
			<div class="ckheadertitle">CAROUSEL CK - <?php echo \Carouselck\CKText::_('CK_STYLES'); ?></div>
		</div>
	</div>
	...
	<div id="filter-bar" class="btn-toolbar input-group">
		<div class="filter-search btn-group pull-left">
			<label for="filter_search" class="element-invisible"><?php echo \Carouselck\CKText::_('JSEARCH_FILTER_LABEL'); ?></label>
			<input type="text" name="filter_search" id="filter_search" placeholder="<?php echo \Carouselck\CKText::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->state->get('filter_search'); ?>" class="form-control" />
		</div>
		<div class="input-group-append btn-group pull-left hidden-phone">
			<button type="submit" class="btn btn btn-primary hasTooltip" title="<?php echo \Carouselck\CKText::_('JSEARCH_FILTER_SUBMIT'); ?>"><i class="icon-search"></i></button>
			<button type="button" class="btn btn-secondary hasTooltip" title="<?php echo \Carouselck\CKText::_('JSEARCH_FILTER_CLEAR'); ?>" onclick="this.form.filter_search.value = '';
					this.form.submit();"><i class="icon-remove"></i></button>
		</div>
		<div class="btn-group pull-right hidden-phone ordering-select">
				<label for="limit" class="element-invisible"><?php echo \Carouselck\CKText::_('JFIELD_PLG_SEARCH_SEARCHLIMIT_DESC'); ?></label>
	...
	<table class="table table-striped" id="itemsList">
		<thead>
			<tr>
				...
				<th width="1%">
					<input type="checkbox" name="checkall-toggle" title="<?php echo \Carouselck\CKText::_('JGLOBAL_CHECK_ALL'); ?>" value="" onclick="Joomla.checkAll(this)" />
				</th>
				<?php } ?>
				<th class='left'>
			...
			foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td>
						...
						<?php /*<a href="<?php echo \Carouselck\CKUri::root(true) . '/administrator/' . $link ?>" class="ckbutton"><?php echo \Carouselck\CKText::_('CK_EDIT'); ?></a>*/ ?>
						<?php } else { ?>
						<a onclick="CKBox.open({handler:'iframe', fullscreen: true, url:'<?php echo \Carouselck\CKUri::root(true) . '/administrator/' . $link ?>'})" href="#"><?php echo $name; ?></a>
						<?php } ?>
					</td>
					<td class="center">
