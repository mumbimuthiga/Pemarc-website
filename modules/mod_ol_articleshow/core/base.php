		...
		public static function getAImages($item, $params, $prefix='imgol'){
			...
			if ( !isset(self::$image_cache[$hash][$item->id]) ){
				...
				if ( count($images_path) == 0){
					$images_path[] = array('src'=> $params->get($prefix.'_placeholder_path', 'modules/mod_mx_coolpage/assets/images/noimage.jpg'), 'class'=>'placeholder');
				}
			...
			foreach ($searchTags as $tag => $regex){
				...
				if ( is_array($m) && isset($m[0]) && count($m[0])){
					foreach ($m[0] as $htmltag){
						$tmp = Utility::parseAttributes($htmltag);
						if ( isset($tmp['src']) ){
							if ($tag == 'input'){
