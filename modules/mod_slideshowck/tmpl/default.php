...
<div class="slideshowck <?php echo $params->get('moduleclass_sfx'); ?> camera_wrap <?php echo $params->get('skin'); ?>" id="camera_wrap_<?php echo $module->id; ?>"<?php echo $width; ?>>
	...
	foreach ($items as $item) {
		...
		// use the minified thumb but don't create it
			...
			if (Slideshowck\CKUri::root(true) && substr($thumbfile, 0, (int)strlen(Slideshowck\CKUri::root(true) . '/')) == (Slideshowck\CKUri::root(true) . '/')) {
				$thumbfile = str_replace(Slideshowck\CKUri::root(true) . '/', '', $thumbfile);
			}
			if (file_exists(JPATH_ROOT . '/' . trim($thumbfile, '/'))) {
				$item->thumb = Slideshowck\CKUri::root(true) . '/' . trim($thumbfile, '/');
			}
		}
...
//					$showcaption = $params->get('usecaption', '1') == '1' && ($item->title || $item->desc);
					...
					<?php if ($link && $linkposition == 'button') { ?>
						<?php echo $startLink . Slideshowck\CKText::_($params->get('linkbuttontext', 'MOD_SLIDESHOWCK_LINK_BUTTON_TEXT')) . '</a>'; ?>
					<?php } ?>
					</div>
				<?php } ?>
			<?php
