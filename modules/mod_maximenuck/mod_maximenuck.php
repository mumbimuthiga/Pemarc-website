...
$behavior = $params->get('behavior', 'mouseover');
	...
	if ($params->get('loadcompiledcss', '0') == '0') {
		if ( file_exists(dirname(__FILE__) . '/themes/' . $theme . '/css/maximenuck.php') ) {
			if ($langdirection == 'rtl' && file_exists(dirname(__FILE__) . '/themes/' . $theme . '/css/maximenuck_rtl.php')) {
				$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/maximenuck_rtl.php?monid=' . $menuID);
			} else {
				$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/maximenuck.php?monid=' . $menuID);
			}
...
display: block !important;
			...
			if ($orientation == 'vertical') {
				if ($langdirection == 'rtl' && file_exists(dirname(__FILE__) . '/themes/' . $theme . '/css/moo_maximenuvck_rtl.css')) {
					$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/moo_maximenuvck_rtl.css');
				} else {
					$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/moo_maximenuvck.css');
				}
				if ($usecss == 1 ) {
					if ($langdirection == 'rtl' && file_exists(dirname(__FILE__) . '/themes/' . $theme . '/css/maximenuvck_rtl.php')) {
						$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/maximenuvck_rtl.php?monid=' . $menuID);
					} else {
						$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/maximenuvck.php?monid=' . $menuID);
					}
				}
			} else {
				if ($langdirection == 'rtl' && file_exists(dirname(__FILE__) . '/themes/' . $theme . '/css/moo_maximenuhck_rtl.css')) {
					$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/moo_maximenuhck_rtl.css');
				} else {
					$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/moo_maximenuhck.css');
				}
				if ($usecss == 1) {
					if ($langdirection == 'rtl' && file_exists(dirname(__FILE__) . '/themes/' . $theme . '/css/maximenuhck_rtl.php')) {
						$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/maximenuhck_rtl.php?monid=' . $menuID);
					} else {
						$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/' . $theme . '/css/maximenuhck.php?monid=' . $menuID);
					}
				}
			}
		}
...
if ($isMaximenuMobilePluginActive) {
	$maximenuplugin = \Maximenuck\CKFof::getPlugin('system', 'maximenuckmobile');
	$pluginParams = new \Maximenuck\CKRegistry($maximenuplugin->params);
...
$isMobileMenuPluginActive = \Joomla\CMS\Plugin\PluginHelper::isEnabled('system', 'mobilemenuck');
...
if ($usejavascript && $params->get('layout', 'default') != '_:flatlist' && $params->get('layout', 'default') != '_:nativejoomla' && $params->get('layout', 'default') != '_:dropselect') {
	if ($debug == true) {
		$document->addScript(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/assets/maximenuck.js');
	} else {
		$document->addScript(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/assets/maximenuck.min.js?ver=10.1.11');
	}

	if ($fxtransition != 'linear' || $fancytransition != 'linear')
		$document->addScript(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/assets/jquery.easing.1.3.js');
	if ($opentype == 'scale' || $opentype == 'puff' || $opentype == 'drop')
		$document->addScript(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/assets/jquery.ui.1.8.js');
	...
	$js = $load . "new Maximenuck('#" . $menuID . "', {"
			...
			. "topfixedoffsetboth : '" . $params->get('topfixedoffsetboth', '0') . "',"
...
// add fancy effect
	if ($orientation == 'horizontal' && $usefancy == 1) {
		// $document->addScript(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/assets/fancymenuck.js');
...
if ((int) $params->get('loadcompiledcss', '0') > 0) {
	...
	$document->addStyleSheet(\Maximenuck\CKUri::base(true) . '/modules/mod_maximenuck/themes/custom/css/maximenuck_' . $menuID . '.css');
}
...
if ($loadfontawesome) {
	...
	display: flex;
	flex-direction: ' . ($params->get('faiconpositionlevel1', 'left') == 'left' || $params->get('faiconpositionlevel1', 'left') == 'right' ? 'row' : 'column') . ';
}

#' . $menuID . ' ul.maximenuck li.maximenuck.level2 span.titreck {
	display: flex;
	flex-direction: ' . ($params->get('faiconpositionlevel2', 'left') == 'left' || $params->get('faiconpositionlevel1', 'left') == 'right' ? 'row' : 'column') . ';
	' . ($params->get('faiconpositionlevel1', 'left') == 'left' ? 'margin-right: 5px;' : '') . '
	' . ($params->get('faiconpositionlevel1', 'left') == 'right' ? 'margin-left: 5px;' : '') . '
}

#' . $menuID . ' .maximenuiconck {
	align-self: center;
	' . ($params->get('faiconpositionlevel1', 'left') == 'left' ? 'margin-right: ' : ($params->get('faiconpositionlevel1', 'left') == 'right' ? 'margin-left: ' : ($params->get('faiconpositionlevel1', 'left') == 'bottom' ? 'margin-top: ' : 'margin-bottom: '))) . Helper::testUnit($params->get('faiconmargin', '5px')) . ';
	' . ($params->get('faiconpositionlevel1', 'left') == 'right' || $params->get('faiconpositionlevel1', 'left') == 'bottom' ? 'order: 1;' : '') . '
}
...
if (($behavior == 'clickclose' || $behavior == 'click') && $params->get('clickclose', '0')) {
	$allCss .= '.maxiclose {
	color: #fff;
...
$allCss .= '#' . $menuID . ' ul.maximenuck li.maximenuck > a:focus, #' . $menuID . ' ul.maximenuck li.maximenuck > span:focus {
    outline: ' . Helper::testUnit($params->get('accessibilty_border_width', '2')) . ' dashed ' . $params->get('accessibilty_border_color', '#ff0000') . ' !important;
}';
}
...
if ( $doCompile ) {
	...
	if (! file_exists(dirname(__FILE__) . '/themes/custom/css/')) {
		mkdir(dirname(__FILE__) . '/themes/custom/css/');
	}
