...
// v1.4.28	- 08/07/25 : add accessibility
...
//	if(opts.loader=='pie' && $.browser.msie && $.browser.version < 9){
	...
	if(opts.navigation==true){
		fakeHover.append(
			'<div class="camera_prev" role="button" tabindex="0" aria-label="' + Joomla.JText._('SLIDESHOWCK_PREVIOUS') + '"><span></span></div>'
			).append(
			'<div class="camera_next" role="button" tabindex="0" aria-label="' + Joomla.JText._('SLIDESHOWCK_NEXT') + '"><span></span></div>'
			);
	}
...
//        wrap.append(newImg);
	...
	for (loopMove=0;loopMove<amountSlide;loopMove++)
	{
		...
		if(allLinks[loopMove]!=''){
			...
			$('.camera_target_content .cameraContent:eq('+loopMove+')',wrap).append('<a class="camera_link" aria-label="' + Joomla.JText._('SLIDESHOWCK_LINK_FOR') + ' : ' + allAlts[loopMove] + '" '+allRels[loopMove]+allTitles[loopMove]+'href="'+allLinks[loopMove]+'" '+dataBox+' target="'+allTargets[loopMove]+'"></a>');
		}
	...
	if(elem.length!=0){
		...
		if($(pagination).length) {
			...
			for (li = 0; li < amountSlide; li++){
				$('.camera_pag_ul',wrap).append('<li class="pag_nav_'+li+'" style="position:relative; z-index:1002" role="button" tabindex="0" aria-label="' + Joomla.JText._('SLIDESHOWCK_SHOw_SLIDE') + ' '+(li+1)+'"><span><span>'+li+'</span></span></li>');
			}
		...
		if($(commands).length) {
			$(commands).append('<div class="camera_play" role="button" tabindex="0" aria-label="' + Joomla.JText._('SLIDESHOWCK_PLAY') + '"></div>').append('<div class="camera_stop" role="button" tabindex="0" aria-label="' + Joomla.JText._('SLIDESHOWCK_PAUSE') + '"></div>');
			if(autoAdv==true){
				$('.camera_play',camera_thumbs_wrap).hide();
				$('.camera_stop',camera_thumbs_wrap).show();
			} else {
